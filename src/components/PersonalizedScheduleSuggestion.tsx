...
const PersonalizedScheduleSuggestion: React.FC<PersonalizedScheduleSuggestionProps> = ({ profile, initialDate, className }) => {
  ...
  const getEnergyColor = (level: number) => {
    const colors = ['bg-destructive/10 text-destructive', 'bg-warning/10 text-warning', 'bg-warning/10 text-warning', 'bg-success/10 text-success', 'bg-primary/10 text-primary'];
  ...
  const getCategoryColor = (category: string) => {
    const categoryStyles: Record<string, {color: string, icon: React.ElementType}> = {
      'work': {color: 'border-primary/20 bg-primary/5', icon: BarChart3},
      'rest': {color: 'border-success/20 bg-success/5', icon: Shield},
      'personal': {color: 'border-secondary/20 bg-secondary/5', icon: User},
      'energy': {color: 'border-warning/20 bg-warning/5', icon: Sparkles},
      'social': {color: 'border-accent/20 bg-accent/5', icon: Users}
    };

    return categoryStyles[category] || {color: 'border-muted bg-muted/50', icon: CalendarClock};
  };
  
  // 获取能量影响对应的图标
  const getEnergyImpactIcon = (impact: string) => {
    switch (impact) {
      case 'high': return <Zap className="h-4 w-4 text-warning" />;
      case 'medium': return <Sparkles className="h-4 w-4 text-primary" />;
      case 'low': return <Wind className="h-4 w-4 text-success" />;
      case 'restorative': return <RefreshCw className="h-4 w-4 text-secondary" />;
