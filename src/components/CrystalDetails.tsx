...
const ElementIcon = ({ elementKey, t }: { elementKey?: string, t: (key: string) => string }) => {
  ...
  switch (element) {
    case 'fire': return <Sun className="h-4 w-4 text-destructive" aria-label={translatedTitle} />;
    case 'water': return <Droplets className="h-4 w-4 text-primary" aria-label={translatedTitle} />;
    case 'air': return <Wind className="h-4 w-4 text-warning" aria-label={translatedTitle} />;
    case 'earth': return <Gem className="h-4 w-4 text-success" aria-label={translatedTitle} />;
    default: return <Zap className="h-4 w-4 text-muted-foreground" aria-label={translatedTitle} />;
  }
};
...
const ChakraIcon = ({ chakraKey, t }: { chakraKey?: string, t: (key: string) => string }) => {
   ...
   switch (chakra) {
    case 'root': return <div className="w-3 h-3 rounded-full bg-destructive" title={translatedTitle} />;
    case 'sacral': return <div className="w-3 h-3 rounded-full bg-warning" title={translatedTitle} />;
    case 'solarplexus': return <div className="w-3 h-3 rounded-full bg-warning" title={translatedTitle} />;
    case 'heart': return <div className="w-3 h-3 rounded-full bg-success" title={translatedTitle} />;
    case 'throat': return <div className="w-3 h-3 rounded-full bg-primary" title={translatedTitle} />;
    case 'thirdeye': return <div className="w-3 h-3 rounded-full bg-secondary" title={translatedTitle} />;
    case 'crown': return <div className="w-3 h-3 rounded-full bg-accent" title={translatedTitle} />;
    default: return <div className="w-3 h-3 rounded-full bg-muted" title={translatedTitle} />;
   }
};


const CrystalDetails: React.FC<CrystalDetailsProps> = ({ crystal }) => {
  const { t, getCrystalDisplayName, normalizeColorToKey } = useLanguage();
  ...
  const displayName = getCrystalDisplayName(crystal.englishName as keyof typeof import('@/lib/crystal-options').crystalTypeMapping); 
