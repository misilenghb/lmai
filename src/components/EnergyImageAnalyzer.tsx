...
import { Lightbulb, Loader2, ScanSearch, BarChartHorizontalBig, Wind, Sun, Droplets } from 'lucide-react';
        ...
        {analysisResult && (
          <Card className="mt-6 bg-background shadow-inner">
            ...
            <CardContent className="space-y-4">
              <Section title={t('common.summary')} icon={() => <LuxuryIcons.Crystal size={16} className={generateLuxuryIconClasses({ size: 'sm', variant: 'default' })} />}>
                <FormattedText text={analysisResult.holisticSummary} />
              </Section>
              <Section title={t('energyExplorationPage.imageAnalyzer.results.energyField')} icon={() => <Wind className="h-4 w-4" />}>
                <FormattedText text={analysisResult.energyFieldAnalysis} />
              </Section>
              <Section title={t('energyExplorationPage.imageAnalyzer.results.chakraAnalysis')} icon={() => <BarChartHorizontalBig className="h-4 w-4" />}>
                <FormattedText text={analysisResult.chakraAnalysis} />
              </Section>
              <Section title={t('energyExplorationPage.imageAnalyzer.results.metaphysicalAssociations')} icon={() => <LuxuryIcons.Crystal size={16} className={generateLuxuryIconClasses({ size: 'sm', variant: 'default' })} />}>
                 <div className="space-y-4">
                    <div>
                        <h5 className="font-semibold flex items-center text-sm mb-1">
                          ...
                          />
                          <span className="ml-1.5">{t('energyExplorationPage.imageAnalyzer.results.zodiac')}</span>
                        </h5>
                    ...
                    <div>
                     ...
                     <div>
                        <h5 className="font-semibold flex items-center text-sm mb-1">
                          ...
                          />
                          <span className="ml-1.5">{t('energyExplorationPage.imageAnalyzer.results.mbtiProfile')}</span>
                        </h5>
