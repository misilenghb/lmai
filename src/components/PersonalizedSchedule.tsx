...
import { Coffee, Moon, Users, Clock } from 'lucide-react';
...
const ICONS: { [key: string]: React.ReactElement } = {
  meditation: <LuxuryIcons.Meditation size={16} className={generateLuxuryIconClasses({ size: 'sm', variant: 'default' })} />,
  focus: <LuxuryIcons.Insight size={16} className={generateLuxuryIconClasses({ size: 'sm', variant: 'default' })} />,
  social: <Users size={16} />,
  relax: <LuxuryIcons.Meditation size={16} className={generateLuxuryIconClasses({ size: 'sm', variant: 'default' })} />,
  default: <LuxuryIcons.Calendar size={16} className={generateLuxuryIconClasses({ size: 'sm', variant: 'default' })} />,
};
      ...
      <CardContent>
        <ul className="space-y-4">
          {schedule.map((item, index) => (
            <li key={index} className="flex items-start space-x-4 p-4 bg-muted/50 rounded-lg shadow-sm">
