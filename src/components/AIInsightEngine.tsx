...
const AIInsightEngine: React.FC<AIInsightEngineProps> = ({
  ...
  const getElementIcon = (element: string) => {
    switch (element.toLowerCase()) {
      case 'fire': return <Flame className="h-4 w-4 text-destructive" />;
      case 'water': return <Droplets className="h-4 w-4 text-primary" />;
      case 'air': return <Wind className="h-4 w-4 text-muted-foreground" />;
      case 'earth': return <Mountain className="h-4 w-4 text-success" />;
      default: return <Sparkles className="h-4 w-4 text-secondary" />;
    }
  };
