...
const SmartScheduleRecommendation: React.FC<SmartScheduleRecommendationProps> = ({
  ...
  const getPeriodIcon = (period: string) => {
    switch (period) {
      case 'morning': return <Sun className="h-4 w-4 text-warning" />;
      case 'afternoon': return <Sunset className="h-4 w-4 text-warning" />;
      case 'evening': return <Moon className="h-4 w-4 text-primary" />;
  ...
  const getActivityTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'social': return 'bg-success/10 text-success';
      case 'rest': return 'bg-muted/50 text-muted-foreground';
      case 'exercise': return 'bg-destructive/10 text-destructive';
      case 'learning': return 'bg-warning/10 text-warning';
      case 'routine': return 'bg-accent/10 text-accent';
      case 'reflection': return 'bg-secondary/10 text-secondary';
      case 'relaxation': return 'bg-success/10 text-success';
